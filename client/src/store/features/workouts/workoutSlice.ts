...
export const fetchWorkout = (workoutId: string): AppThunk => async (dispatch: Dispatch) => {
  try {
...
export const createWorkout = (userId: string, date: Date, exercises: Exercise[]): AppThunk => async (
  dispatch: Dispatch
) => {
  try {
    dispatch(getWorkoutStart());
    const response = await axios.post(`API_ENDPOINT`, { userId, date, exercises });
    dispatch(getWorkoutSuccess(response.data));
  } catch (error) {
    dispatch(getWorkoutFailure((error as Error).message));
  }
...
export const { getWorkoutStart, getWorkoutSuccess, getWorkoutFailure } = workoutsSlice.actions;
